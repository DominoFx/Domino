...
int EnttecDMXUSB::sendPacket(int pkt_type, char *data, int length)
{
    ...
    if(detected == false)
            fprintf(stderr, " Interface %s non d�tect�e\n", nomInterfaces[interface]);
    else    fprintf(stderr, " Interface %s d�tect�e\n", nomInterfaces[interface]);
    #endif
...
bool EnttecDMXUSB::openPort(string port_use/*="/dev/ttyUSB0"*/)
{
    ...
    dmxout_length = NB_CANAUX_MAX+1; //+ le startcode
    comport = port_use;
    device_mode = PKT_DMXIN; /* par d�faut */
    ...
    //recupere la configuration de l'interface d�tect�e et termine
    if (detected)
    {
        if(interface == DMX_USB_PRO)
        {
